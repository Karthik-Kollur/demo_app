import 'package:demo_app/widggets/favorite_Model.dart';
...
class DBHelper {
  ...
  Future<Cart> insertToFav(Cart cart)async{
    print(cart.toMap());
    var dbClient = await db ;
    await dbClient!.insert('cart', cart.toMap());
    return cart ;
  ...
  Future<List<Cart>> getFavList()async{
    var dbClient = await db ;
    final List<Map<String , Object?>> queryResult =  await dbClient!.query('cart');
    return queryResult.map((e) => Cart.fromMap(e)).toList();
  ...
  Future<int> updateQuantityFav(Favorite fav)async{
    var dbClient = await db ;
    return await dbClient!.update(
        'cart',
        fav.toMap(),
